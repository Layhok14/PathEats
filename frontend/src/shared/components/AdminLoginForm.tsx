...
import { consumeSessionNotice } from "../utils/authRedirect";
...
export default function AdminLoginForm({ title, subtitle, redirectMap, defaultRedirect, expectedRoles }: Props) {
  const { login } = useAuth();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const result = await login(email, password, expectedRoles);
      navigate(redirectMap[result.user.role_scope] || defaultRedirect, { replace: true });
    } catch (err: any) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#f8fafc] px-4">
      <div className="w-full max-w-sm bg-white rounded-2xl border border-[#e2e8f0] shadow-sm p-8">
        <div className="text-center mb-8">
          ...
          <h1 className="text-lg font-bold text-[#0b1c30]">{title}</h1>
