...
import type { Vendor, Cuisine } from "../../shared/types/index";
 ...
 */
export async function getVendorsAlongRoute(params: SearchParams): Promise<ScoredVendor[]> {
  const { routePoints, radiusMetres, cuisine = "All", maxPrice = 4, openNow = false, query = "" } = params;

  console.log("[vendorService] getVendorsAlongRoute", { radiusMetres, cuisine, maxPrice, openNow });

  const q = query.toLowerCase().trim();

  const scored = ALL_VENDORS
    .map((v) => {
      const dist = distToRouteM(v.lat, v.lng, routePoints);
      if (dist > radiusMetres) return null;
      return { ...v, dist_m: Math.round(dist), final_score: calcScore(v.price_range, dist, v.rating, v.wait_time_est) };
    })
    .filter((v): v is ScoredVendor => v !== null)
    .filter((v) => cuisine === "All" || v.cuisine === cuisine)
    .filter((v) => v.price_range <= maxPrice)
    .filter((v) => !openNow || v.open_now)
    .filter((v) => {
      if (!q) return true;
      return v.name.toLowerCase().includes(q) || v.menu.some((m) => m.name.toLowerCase().includes(q));
    })
