...
export function NavRail({ activeTab, onTabChange, showSettings, onSettingsToggle, onProfileOpen, onHomeReset }) {
  ...
  return (
    <nav className="flex flex-col items-center pt-4 pb-4 gap-1 shrink-0 h-full relative"
      ...
      <AnimatePresence>
        {showSettings && (
          <motion.div initial={{ opacity: 0, x: -8, scale: 0.95 }} animate={{ opacity: 1, x: 0, scale: 1 }}
            ...
            style={{ width: 220, background: darkMode ? "#111828" : "#fffdf7", boxShadow: "0 12px 40px rgba(0,0,0,0.45)", border: `1px solid ${tm.border}` }}>

            <div className="px-4 py-3.5 flex items-center gap-3" style={{ borderBottom: `1px solid ${tm.border}` }}>
              <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0" style={{ background: "#22c55e" }}>
                <User size={16} className="text-white" />
              </div>
              <div>
                <div className="text-sm font-semibold" style={{ color: tm.text1 }}>
            ...
            <div className="py-1.5">
              <button onClick={() => { onSettingsToggle(); onProfileOpen(); }}
                ...
                <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ background: tm.surface2 }}>
                  <User size={13} style={{ color: tm.text3 }} />
                </div>
                <div className="flex-1 text-left">
                  <div className="text-xs font-medium" style={{ color: tm.text1 }}>View Profile</div>
                  <div className="text-[10px]" style={{ color: tm.text4 }}>Edit your info</div>
                </div>
              ...
              <div className="flex items-center gap-3 px-4 py-2.5">
                <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ background: tm.surface2 }}>
                  {darkMode ? <Moon size={13} style={{ color: tm.text3 }} /> : <Sun size={13} style={{ color: tm.text3 }} />}
                </div>
                <div className="flex-1">
                  <div className="text-xs font-medium" style={{ color: tm.text1 }}>Theme</div>
                  <div className="text-[10px]" style={{ color: tm.text4 }}>{darkMode ? "Dark" : "Light"}</div>
                </div>
                <button onClick={() => setDarkMode(d => !d)}
                  className="w-10 h-5 rounded-full relative transition-colors shrink-0"
                  style={{ background: darkMode ? "#22c55e" : tm.surface3 }}>
                  <div className="absolute top-[2px] w-4 h-4 bg-white rounded-full shadow transition-all duration-200"
            ...
            <div style={{ borderTop: `1px solid ${tm.border}` }}>
              <button onClick={logout}
                ...
                <span className="text-xs font-medium text-red-400">Sign out</span>
              </button>
            </div>
          </motion.div>
        )}
