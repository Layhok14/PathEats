 ...
 */
export function SearchHistoryPanel({ history, onReplay, onDelete, onClearAll }) {
  const { tm } = useTheme();
      ...
      <div className="shrink-0 px-4 pt-5 pb-3 flex items-center justify-between"
        ...
        <div>
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <span className="text-sm font-bold" style={{ color: tm.text1 }}>Search History</span>
          </div>
        ...
        {history.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 gap-3 px-4">
            ...
            <p className="text-xs text-center" style={{ color: tm.text4 }}>
              No searches yet.<br />Enter a route and tap Find Route to start.
            </p>
          </div>
        ) : (
          <div className="py-2">
            {history.map(entry => (
              <div key={entry.id}
                ...
                <button onClick={() => onReplay(entry)} className="flex-1 min-w-0 text-left">
                  <div className="text-[12px] font-semibold truncate" style={{ color: tm.text1 }}>
                    {entry.origin}
                  </div>
                  <div className="text-[11px] truncate mt-0.5" style={{ color: tm.text3 }}>
                    → {entry.dest}
                  </div>
                  <div className="text-[10px] flex items-center gap-1 mt-1" style={{ color: tm.text5 }}>
