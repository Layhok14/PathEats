...
import { PHNOM_PENH_CENTER, ROADMAP_TILE, DARK_TILE } from "../../shared/constants/appConfig";
...
function makeVendorIcon(rank, score, selected) {
  ...
  return L.divIcon({
    ...
    iconSize: [32, 32], iconAnchor: [4, 32],
  });
}
...
export function useLeafletMap({ darkMode, routePoints, routeReady, editRouteMode, scoredVendors, selectedVendorId, onSelectVendor, onWaypointAdded }) {
  ...
  useEffect(() => {
    ...
    const tile = L.tileLayer(darkMode ? DARK_TILE : ROADMAP_TILE, { maxZoom: 19 }).addTo(map);
  ...
  useEffect(() => {
    ...
    const line = L.polyline(routePoints, { color: "#3B82F6", weight: 4, opacity: 0.85 }).addTo(map);
    routeLayerRef.current = line;

    // Icons created here so Leaflet is guaranteed to be DOM-ready
    const A = L.marker(routePoints[0],                     { icon: makeEndpointIcon("#10b981", "A"), draggable: true }).addTo(map);
    const B = L.marker(routePoints[routePoints.length - 1], { icon: makeEndpointIcon("#ef4444", "B"), draggable: true }).addTo(map);
    endpointMarkersRef.current = [A, B];

    const ghost = L.circleMarker(routePoints[0], { radius: 7, color: "#3B82F6", fillColor: "#3B82F6", fillOpacity: 0.5, weight: 2 });
    line.on("mousemove", e => { if (!editModeRef.current) return; ghost.setLatLng(e.latlng); if (!map.hasLayer(ghost)) ghost.addTo(map); });
    line.on("mouseout",  () => ghost.remove());
    line.on("click",     e => { if (editModeRef.current) onWaypointAdded(e.latlng.lat, e.latlng.lng); });
  ...
  useEffect(() => {
    ...
    scoredVendors.forEach((v, i) => {
      const icon   = makeVendorIcon(i + 1, v.final_score, selectedVendorId === v.id);
      const marker = L.marker([v.lat, v.lng], { icon }).addTo(mapRef.current).on("click", () => onSelectVendor(v));
