    ...
    <div className="flex h-screen w-screen overflow-hidden" style={{ background: "#f8fafc" }}>
      <aside className="w-[260px] flex flex-col h-full shrink-0" style={{ background: "#004b1e" }}>
        <div className="flex items-center gap-3 px-6 py-5" style={{ borderBottom: "1px solid rgba(188,203,185,0.15)" }}>
          ...
          <div>
            <p className="text-white font-bold text-[18px] leading-tight">PathEat</p>
            <p className="text-[11px]" style={{ color: "#bec6e0" }}>Management Portal</p>
          </div>
        </div>
        <nav className="flex-1 px-4 py-5 flex flex-col gap-1 overflow-y-auto">
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium transition-all ${isActive ? "bg-white/15 text-white" : "text-[#bec6e0] hover:bg-white/10 hover:text-white"}`
              }
            >
