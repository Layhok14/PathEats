...
import {
  ...
  CheckCircle, XCircle, LogIn, Bug, Database, Pencil, Trash2, Plus, Clock
} from "lucide-react";
...
export default function DeveloperActivityPage() {
  ...
  const loadCrudLogs = useCallback(async () => {
    setCrudLoading(true);
    try {
      const data = await getAuditLogsByRole("DEVELOPER_ADMIN", 200);
      setCrudLogs(data);
    } catch { setCrudLogs([]); }
  ...
  const handleRefresh = () => {
    setRefreshing(true);
    Promise.all([loadCrudLogs(), loadQueryHistory(), loadActivityLog()])
  ...
  const recentEvents = useMemo(() => {
    ...
    crudLogs.slice(0, 50).forEach((log) => {
      const { label, color } = getCrudInfo(log.action);
      const actorName = [log.actorFirstName, log.actorLastName].filter(Boolean).join(" ") || log.actorEmail || "—";
      events.push({ time: log.createdAt, type: label, typeColor: color, actor: actorName, details: `${log.targetType || "—"}` });
    });
      ...
      <div className="flex-1 p-8 flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[14px] text-[#64748b] mt-1">Monitor all database traffic, queries, and system events.</p>
          </div>
          <button
        ...
        {mainFilter === "recent" && (
          <div className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-[#f1f5f9] flex items-center gap-2">
              <Clock size={14} className="text-[#006e2f]" />
                    ...
                    {recentEvents.map((ev, i) => (
                      <tr key={i} className="border-t border-[#f1f5f9] hover:bg-[#f8fafc] transition-colors">
                        <td className="px-5 py-3 text-[11px] text-[#64748b] whitespace-nowrap">
                          <span className="inline-flex items-center gap-1"><Clock size={11} /> {timeAgo(ev.time)}</span>
                        </td>
                ...
                <table className="w-full min-w-[800px]">
                  ...
                  <tbody>
                    {filteredCrud.map((entry) => {
                      const { label, color, icon: ActionIcon } = getCrudInfo(entry.action);
                      return (
                      <tr key={entry.id} className="border-t border-[#f1f5f9] hover:bg-[#f8fafc] transition-colors">
                        <td className="px-5 py-3 text-[11px] text-[#64748b] whitespace-nowrap">
                          <span className="inline-flex items-center gap-1"><Clock size={11} /> {timeAgo(entry.createdAt)}</span>
                        </td>
                        <td className="px-5 py-3">
                ...
                <table className="w-full min-w-[900px]">
                  ...
                  <tbody>
                    {filteredHistory.map((entry) => {
                      ...
                      <tr key={entry.id} className="border-t border-[#f1f5f9] hover:bg-[#f8fafc] transition-colors">
                        <td className="px-5 py-3 text-[11px] text-[#64748b] whitespace-nowrap">
                          <span className="inline-flex items-center gap-1"><Clock size={11} /> {timeAgo(entry.executed_at)}</span>
                        </td>
