...
import { ChevronLeft, Check, MapPin, Expand } from "lucide-react";
...
function LocationStep({ form, onChange }: { form: StallFormData; onChange: (f: StallFormData) => void }) {
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem("admin_stall_create_location");
    if (saved) {
      sessionStorage.removeItem("admin_stall_create_location");
      try {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div style={{ position: "relative" }}>
        ...
        <button
          type="button"
          onClick={() => navigate(`/admin/restaurants/stall/create?lat=${coords.lat}&lng=${coords.lng}`)}
          style={{
...
export default function AdminStallCreatePage() {
  const navigate = useNavigate();
  const { vendorId, lat, lng } = useParams<{ vendorId?: string; lat?: string; lng?: string }>();
  ...
  useEffect(() => {
    ...
    if (lat && lng) {
      ...
      if (!isNaN(latNum) && !isNaN(lngNum)) {
        setForm(f => ({ ...f, location: { ...f.location, latitude: latNum, longitude: lngNum } }));
      }
    }
  }, [vendorId, lat, lng]);
  ...
  async function handleConfirm() {
    try {
      ...
      console.error("[AdminStallCreatePage] Failed to create stall:", err);
      toast.error("Failed to create stall. Check your connection and try again.");
    }
  }
  ...
  return (
    <div className="p-6 flex flex-col gap-5 max-w-3xl">
      <button onClick={() => navigate("/admin/restaurants")} style={{ display: "flex", alignItems: "center", gap: "4px", background: "none", border: "none", cursor: "pointer", color: "var(--brand-text-muted)", fontFamily: "Poppins, sans-serif", fontSize: "14px" }}>
        <ChevronLeft size={16} /> Back to Restaurants
      </button>

      <div>
        <h1 style={{ fontFamily: "Poppins, sans-serif", fontSize: "26px", fontWeight: 700, color: "var(--brand-text-dark)" }}>Create New Stall</h1>
        <p style={{ fontFamily: "Poppins, sans-serif", fontSize: "14px", color: "var(--brand-text-muted)", marginTop: "4px" }}>Set up a new food stall with details, menu items, and location.</p>
      </div>
      ...
      <div style={cardStyle}>
        {step === 0 && (
          <>
            ...
            <StallInfoForm form={form} onChange={setForm} vendors={allVendors} categories={allCategories} />
          </>
        )}
      ...
      <div className="flex justify-between">
        <button style={btnOutline} onClick={() => step === 0 ? navigate("/admin/restaurants") : setStep(step - 1)}>
          {step === 0 ? "Cancel" : "Back"}
        </button>
        {step < 3 ? (
          <button
          ...
          <button style={{ ...btnPrimary, display: "flex", alignItems: "center", gap: "8px" }} onClick={handleConfirm} disabled={loading}>
            <Check size={16} /> {loading ? "Creating..." : "Confirm & Create Stall"}
          </button>
        )}
      </div>
      ...
      {showSuccess && (
        <SuccessModal
          ...
          onGoBack={() => navigate("/admin/restaurants")}
          backLabel="Go to Restaurants"
          continueLabel="Create Another"
        />
      )}
    </div>
  );
