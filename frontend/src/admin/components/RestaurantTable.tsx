  ...
  return (
    <div className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm overflow-hidden">
      <div className="flex items-start justify-between px-6 py-4 border-b border-[#f1f5f9] flex-wrap gap-3">
        <div><h2 className="text-[16px] font-semibold text-[#0b1c30]">Active Directory</h2><p className="text-[12px] text-[#64748b]">Manage and monitor all currently listed vendors</p></div>
        <div className="flex items-center gap-2 flex-wrap">
          {CATEGORIES.map((c) => (<button key={c} onClick={() => { setActiveCategory(c); setPage(1); }} className={`px-3 py-1.5 rounded-lg text-[12px] font-medium transition-colors ${activeCategory === c ? "bg-[#006e2f] text-white" : "text-[#64748b] border border-[#e2e8f0] hover:bg-gray-50"}`}>{c}</button>))}
        </div>
      </div>
      <table className="w-full">
        <thead><tr className="bg-[#f8fafc]">{["RESTAURANT", "LOCATION", "CATEGORY", "STATUS", "RATING", "ACTIONS"].map((h) => (<th key={h} className="px-6 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-[#64748b]">{h}</th>))}</tr></thead>
        <tbody>
          {visible.map((r) => (
