...
export function MenuItemsPage() {
  ...
  useEffect(() => {
    if (!deleteTarget) { setDeleteLinkCount(null); return; }
    api.get(`/vendor/items/${deleteTarget.id}/links`)
      .then(({ data: res }) => setDeleteLinkCount(res.data?.count ?? 0))
      .catch(() => {});
  }, [deleteTarget]);
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      ...
      {/* Delete confirm */}
      {deleteTarget && (
        <Dialog open onOpenChange={() => { setDeleteTarget(null); setDeleteLinkCount(null); }}>
          <DialogContent>
            <DialogHeader>
            ...
            {deleteLinkCount !== null && deleteLinkCount > 0 && (
              <div style={{ fontFamily: "Poppins, sans-serif", fontSize: "13px", color: "#d4183d", background: "#d4183d15", padding: "10px 14px", borderRadius: "6px", marginTop: "8px" }}>
                This item is linked to {deleteLinkCount} stall(s).
              </div>
            )}
            <DialogFooter>
              <button onClick={() => { setDeleteTarget(null); setDeleteLinkCount(null); }} style={{ padding: "10px 20px", borderRadius: "6px", border: "1px solid var(--brand-card-border)", background: "var(--card)", fontFamily: "Poppins, sans-serif", cursor: "pointer" }}>Cancel</button>
