...
| **Service** | `UserService.js` | `getProfile(userId)`, `updateProfile(userId, data)`, `getFavorites(userId)`, `addFavorite(userId, vendorId)`, `removeFavorite(userId, vendorId)` |
| **Controller** | `UserController.js` | `getProfile`, `updateProfile`, `listFavorites`, `addFavorite`, `removeFavorite`, `getHistory` |
| **Routes** | `userRoutes.js` | `GET/PUT /api/user/profile`, `GET/POST/DELETE /api/user/favorites`, `GET /api/user/routes/log`, `GET /api/user/alerts` |

**Person 1 — Customer Service:**

| Layer | Files to Create | What to Implement |
|-------|----------------|-------------------|
| **Service** | `CsService.js` | `listTickets(status, priority)`, `getTicket(id)`, `updateTicket(id, data)`, `listDisputes(type)`, `approveVendor(vendorId)`, `moderateReview(reviewId)` |
...
| Layer | Files to Create | What to Implement |
|-------|----------------|-------------------|
| **Model** | `VendorModel.js` | Shape: id, name, category, location (PostGIS), rating, wait_time_est, open_now, hours, address, photo_url, description, owner_id (FK users) |
| **Model** | `OrderModel.js` | Shape: id, vendor_id, user_id, items (JSONB), status, total, created_at |
| **Repository** | `VendorRepository.js` | `findByOwner(ownerId)`, `findById(id)`, `create(data)`, `update(id, data)`, `spatialSearch(origin, dest, radius, cuisine, maxPrice)` — the gold-standard PostGIS query |
| **Repository** | `OrderRepository.js` | `findByVendor(vendorId)`, `findById(id)`, `updateStatus(id, status)` |
| **Service** | `VendorService.js` | `getDashboard(ownerId)`, `getMenu(vendorId)`, `updateMenu(vendorId, items)`, `getOrders(vendorId, status)`, `updateOrderStatus(orderId, status)` |
...
| Layer | Files to Create | What to Implement |
|-------|----------------|-------------------|
| **Service** | `DevService.js` | `getSystemHealth()`, `getErrorLogs(severity, page)`, `getDbTables()`, `triggerBackup(type)`, `getBackups()`, `runSeed()` |
| **Controller** | `DevController.js` | `getHealth`, `getLogs`, `getDbTables`, `createBackup`, `listBackups`, `runSeed`, `getApiMetrics`, `getDbPerf` |
| **Routes** | `devRoutes.js` | All `GET/POST /api/dev/*` with `restrictToRoles("DEVELOPER_ADMIN")` |

### Person 3 — Keo Seavpav (Global Admin Domain + Auth)

| Layer | Files to Create | What to Implement |
|-------|----------------|-------------------|
| **Auth Service** | `AuthService.js` | `register(data)` — hash password via bcryptjs, insert user, return JWT. `login(email, password)` — verify hash, return JWT. `forgotPassword(email)` — generate OTP, store in DB. `verifyOtp(email, otp)` — validate OTP. `resetPassword(email, otp, newPassword)` |
| **Auth Controller** | `AuthController.js` | `register`, `login`, `forgotPassword`, `verifyOtp`, `resetPassword` |
| **Auth Routes** | `authRoutes.js` | `POST /api/auth/register`, `POST /api/auth/login`, `POST /api/auth/forgot-password`, `POST /api/auth/verify-otp`, `POST /api/auth/reset-password` |
| **Service** | `AdminService.js` | `listUsers(filters, page)`, `updateUserRole(userId, role)`, `banUser(userId)`, `unbanUser(userId)`, `listVendors(filters, page)`, `approveVendor(vendorId)`, `getSystemConfig()`, `updateSystemConfig(data)` |
| **Controller** | `AdminController.js` | `listUsers`, `updateUserRole`, `banUser`, `unbanUser`, `listVendors`, `approveVendor`, `getSystemConfig`, `updateSystemConfig`, `listAuditLogs` |
| **Routes** | `adminRoutes.js` | All `GET/POST /api/admin/*` with `restrictToRoles("GLOBAL_ADMIN")` |
| **Middleware** | `authMiddleware.js` | JWT verification from `Authorization: Bearer <token>` |
| **Middleware** | `rbacGuard.js` | `restrictToRoles(...roles)` |
