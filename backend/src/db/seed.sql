...
CREATE UNIQUE INDEX idx_menu_items_owner_normalized_name
  ...
  FOR i IN 1..1000 LOOP
    ...
    FOR j IN 1..item_count LOOP
      ...
      WITH created_item AS (
        INSERT INTO menu_items (owner_id, name, description, default_price, category)
        VALUES (owner_for_place[i], item_name, 'Freshly prepared ' || item_name, round(item_price::numeric, 2), item_cat)
        ON CONFLICT (owner_id, lower(regexp_replace(trim(name), '[[:space:]]+', ' ', 'g')))
        DO UPDATE SET name = menu_items.name
        RETURNING id
      )
...
INSERT INTO "role" (name, base_scope, table_privileges, system_capabilities, grant_option, is_system) VALUES
  ('CONSUMER', 'CONSUMER', '{"users":["SELECT","UPDATE"],"places":["SELECT"],"menu_items":["SELECT"],"place_menu_items":["SELECT"],"reviews":["SELECT","INSERT","UPDATE","DELETE"],"routes":["SELECT","INSERT","UPDATE","DELETE"],"bookmarks":["SELECT","INSERT","UPDATE","DELETE"],"search_history":["SELECT","INSERT","DELETE"]}'::jsonb, '[]'::jsonb, FALSE, TRUE),
  ('VENDOR', 'VENDOR', '{"users":["SELECT","UPDATE"],"places":["SELECT","INSERT","UPDATE","DELETE"],"place_categories":["SELECT"],"place_hours":["SELECT","INSERT","UPDATE","DELETE"],"place_images":["SELECT","INSERT","UPDATE","DELETE"],"menu_items":["SELECT","INSERT","UPDATE","DELETE"],"menu_item_images":["SELECT","INSERT","UPDATE","DELETE"],"place_menu_items":["SELECT","INSERT","UPDATE","DELETE"],"reviews":["SELECT"]}'::jsonb, '[]'::jsonb, FALSE, TRUE),
